...
<body>
	<div style="width:700px; margin:0 auto;">
		...
		$result_count = mysqli_query($con, "SELECT COUNT(*) As total_records FROM t_gl_bluetooth_task_master where emp_id='$eid' and date >= DATE_ADD(NOW(), INTERVAL -$load_more MONTH)");
		...
		$result = mysqli_query($con, "SELECT * FROM t_gl_bluetooth_task_master where emp_id='$eid' and date >= DATE_ADD(NOW(), INTERVAL -$load_more MONTH) ORDER BY date DESC LIMIT $offset, $total_records_per_page");
		?>
		...
		<div style='padding: 10px 20px 0px; border-top: dotted 1px #CCC;'>
			<strong>
				<?php
				if ($total_records > 0) {
					echo "Page " . $page_no . " of " . $total_no_of_pages;
				} else {
					echo "No Tasks Found";
				}
				?>
